...
def parse_iv_file(
    ...
    iv_re = re.compile(
        ...
        r'[\t ]+'
        r'(-?\d+\.\d+(?:[eE][+-]?\d+)?)\*?'     # Optionnel aussi sur le courant au cas où
    )
    param_re = {
        'Voc':   re.compile(r'^Voc\s*=\s*(-?\d+\.\d+(?:[eE][+-]?\d+)?)'),
        'Jsc':   re.compile(r'^Jsc\s*=\s*(-?\d+\.\d+(?:[eE][+-]?\d+)?)'),
    ...
    def flush():
        ...
        if current_is_dark:
            ...
            solar_keys = ('Voc', 'Jsc', 'FF', 'eta', 'V_MPP', 'J_MPP')
            if len(current_params) != 6:
                missing = [k for k in solar_keys if k not in current_params]
                print(f"[parse_iv_file] Attention : paramètres solaires manquants {missing} "
                      f"pour batch_params={current_batch_params}, temperature={current_temperature}. "
                      f"Ligne conservée avec '0' pour les valeurs manquantes.")
            solar_params = [current_params.get(k, '0') for k in solar_keys]
